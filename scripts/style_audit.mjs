...
const CODEBASE_MIN_LINE_LIMIT = 30;
const MIN_AT_OR_ABOVE_CODEBASE_MIN_PERCENT = 90;
...
function isMinLineCheckExempt(relativePath) {
...
function scanJsTs(
    ...
    for (const rawLine of lines) {
        ...
        while (match !== null) {
            consoleHits.push(
                `${relativePath}:${lineNumber} console.${match[1]}`,
            );
...
function scanJsTsDocumentation(relativePath, content, documentationHits) {
    ...
    if (!hasJsTsModuleDoc(lines)) {
        documentationHits.push(
            `${relativePath}:1 module missing top-level JSDoc`,
        );
    }
...
function hasPythonDocBelow(lines, declarationLineIndex) {
    for (
        let index = declarationLineIndex + 1;
        index < lines.length;
        index += 1
    ) {
        const rawLine = lines[index];
...
function run() {
    ...
    let analyzed = 0;
    let atOrAboveCodebaseMinimum = 0;
    let codebaseMinEligible = 0;
    let underSoft = 0;
    let typesAnalyzed = 0;
    let typesAtOrAboveMinimum = 0;
    let typesMinEligible = 0;
    ...
    for (const filePath of collectFiles()) {
        ...
        const minCheckExempt = isMinLineCheckExempt(relativePath);

        analyzed += 1;
        if (!minCheckExempt) {
            codebaseMinEligible += 1;
            if (lineCount >= CODEBASE_MIN_LINE_LIMIT) {
                atOrAboveCodebaseMinimum += 1;
            } else {
                codebaseUnderMinimum.push({
                    lines: lineCount,
                    path: relativePath,
                });
            }
        ...
        if (lineCount > SOFT_LINE_LIMIT) {
            overSoftLimit.push({ lines: lineCount, path: relativePath });
        }
        if (lineCount > HARD_LINE_LIMIT && !isElectronTypesPath(relativePath)) {
            overHardLimit.push({ lines: lineCount, path: relativePath });
        }
    ...
    sortByLinesAscending(codebaseUnderMinimum);
    sortByLinesAscending(typesUnderMinimum);

    let atOrAboveCodebaseMinimumPercent = 100;
    let underSoftPercent = 100;
    let typesAtOrAboveMinimumPercent = 100;
    if (codebaseMinEligible > 0) {
        atOrAboveCodebaseMinimumPercent =
            (atOrAboveCodebaseMinimum / codebaseMinEligible) * 100;
    }
    if (analyzed > 0) {
        underSoftPercent = (underSoft / analyzed) * 100;
    }
    if (typesMinEligible > 0) {
        typesAtOrAboveMinimumPercent =
            (typesAtOrAboveMinimum / typesMinEligible) * 100;
    }

    process.stdout.write("Style audit report\n");
    process.stdout.write(`Analyzed files: ${analyzed}\n`);
    process.stdout.write(
        `Eligible files at or above ${CODEBASE_MIN_LINE_LIMIT} lines: ${atOrAboveCodebaseMinimum}/${codebaseMinEligible} (${atOrAboveCodebaseMinimumPercent.toFixed(1)}%)\n`,
    );
    process.stdout.write(
        `Files under ${SOFT_LINE_LIMIT} lines: ${underSoft}/${analyzed} (${underSoftPercent.toFixed(1)}%)\n`,
    );
    process.stdout.write(
        `Eligible electron/types files at or above ${TYPES_MIN_LINE_LIMIT} lines: ${typesAtOrAboveMinimum}/${typesMinEligible} (${typesAtOrAboveMinimumPercent.toFixed(1)}%)\n`,
    );

    printSection(`Files over ${SOFT_LINE_LIMIT} lines`, overSoftLimit);
    printSection(`Files over ${HARD_LINE_LIMIT} lines`, overHardLimit);
    printSection(
        `Files under ${CODEBASE_MIN_LINE_LIMIT} code lines`,
        codebaseUnderMinimum,
    );
    printSection(
    ...
    if (
        atOrAboveCodebaseMinimumPercent <
        MIN_AT_OR_ABOVE_CODEBASE_MIN_PERCENT
    ) {
        failures.push(
            `Files at or above ${CODEBASE_MIN_LINE_LIMIT} lines below ${MIN_AT_OR_ABOVE_CODEBASE_MIN_PERCENT}%: ${atOrAboveCodebaseMinimumPercent.toFixed(1)}%`,
        );
    }
    ...
    if (consoleHits.length > 0) {
        failures.push(
            `Disallowed console methods found: ${consoleHits.length}`,
        );
    }
