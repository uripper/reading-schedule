...
pub fn read_state_from_json(data_directory: &Path) -> Option<LoadResult> {
    ...
    if let Some(state) = read_json_object_file(&primary_path) {
    ...
    read_json_object_file(&backup_path).map(|state| LoadResult {
...
fn read_json_object_file(path: &Path) -> Option<Value> {
    let text = fs::read_to_string(path).ok()?;
    let parsed = serde_json::from_str::<Value>(strip_utf8_bom(&text)).ok()?;
