...
export function createZoomShortcutHandler(
  ...
  return (event: KeyboardEvent): boolean => {
    ...
    if (isZoomInShortcut(event)) {
      ...
      RUN_DETACHED(RUN_ZOOM_COMMAND(async () => await plannerApi.zoomIn()));
      return true;
    }
    if (isZoomOutShortcut(event)) {
      event.preventDefault();
      RUN_DETACHED(RUN_ZOOM_COMMAND(async () => await plannerApi.zoomOut()));
      return true;
    }
    if (isZoomResetShortcut(event)) {
      event.preventDefault();
      RUN_DETACHED(RUN_ZOOM_COMMAND(async () => await plannerApi.zoomReset()));
      return true;
    }
    return false;
  };
