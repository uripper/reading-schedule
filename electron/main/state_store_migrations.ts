...
export function migrateLoadedState(
    ...
    throw new Error(
        `Unsupported saved state version: ${VERSION}. ` +
            `This application only supports state versions ${CURRENT_STATE_VERSION} (current) and ${LEGACY_STATE_VERSION} (legacy). ` +
            "This may indicate data saved by a newer or incompatible version of the app, or corrupted saved data. " +
            "Try updating the app to the latest version. If the issue persists, reset or delete the saved planner state and try again.",
    );
}
