...
import { SORT_BY_ESTIMATED_FINISH, SORT_BY_TITLE, sortBooks } from '../dist/renderer/books/sort.js';
...
test('sortBooks by estimated finish includes finished read books in date order', () => {
  const books = [
    ...
    baseBook({ book_id: 'book-2', title: 'Ice', status: 'read', finished_at: '2026-01-10' }),
    baseBook({ book_id: 'book-3', title: 'White Noise', status: 'read', finished_at: '2026-01-20' }),
  ];
  const rows = [
    { book_id: 'book-1', date: '2026-12-22', session_index: 1 },
  ];
  ...
  assert.deepEqual(sorted.map((book) => book.book_id), ['book-2', 'book-3', 'book-1']);
});
