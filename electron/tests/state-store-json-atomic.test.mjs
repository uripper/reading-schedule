...
import { readStateFromJson, writeStateToJson } from "../dist/main/state_store_json.js";
import {
  jsonStateBackupPath,
  jsonStatePath,
...
test("JSON store rotates backup and recovers from corrupted primary", () => {
  ...
  try {
    const firstState = { settings: { start_date: "2026-01-01" }, books: [] };
    const secondState = {
...
test("Facade emits fresh-warning when persisted JSON artifacts are unreadable", () => {
  const userDataDir = tempUserDataDir();
  try {
    assert.equal(writeStateToJson(userDataDir, { settings: {}, books: [] }).ok, true);
    assert.equal(writeStateToJson(userDataDir, { settings: {}, books: [] }).ok, true);
