...
import { mergeScheduleRows, pruneScheduleCompletions } from '../dist/renderer/app/schedule_preserve.js';
...
test('mergeScheduleRows preserves locked-day rows even when book is no longer in future plan', () => {
  ...
  assert.ok(bookIds.includes('book-complete'), 'locked-day rows should be preserved');
  assert.ok(bookIds.includes('book-active'), 'active book rows should be preserved');
});
...
test('mergeScheduleRows keeps past-day rows for books still in new schedule', () => {
  ...
  const previousRows = [
    row({ date: yesterday, session_index: 1, book_id: 'book-1', words_planned: 500 }),
    row({ date: tomorrow, session_index: 1, book_id: 'book-1', words_planned: 500 }),
  ];
...
test('mergeScheduleRows preserves today rows while still rebuilding tomorrow onward', () => {
  ...
  const previousRows = [
    row({ date: today, session_index: 1, book_id: 'book-1', words_planned: 500 }),
    row({ date: tomorrow, session_index: 1, book_id: 'book-1', words_planned: 500 }),
  ];

  const nextRows = [
    row({ date: tomorrow, session_index: 1, book_id: 'book-1', words_planned: 700 }),
  ];
