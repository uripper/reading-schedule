...
test('buildStatsSnapshot combines planned and already-read finishes for current year', () => {
  ...
  const snapshot = buildStatsSnapshot({
    books: [
      book({ book_id: 'book-1', status: BOOK_STATUS_IN_PROGRESS, progress_percent: 30 }),
      book({ book_id: 'book-2', status: BOOK_STATUS_TO_READ, progress_percent: 0 }),
      book({ book_id: 'book-3', status: BOOK_STATUS_READ, progress_percent: 100, finished_at: febFirst }),
    ],
    sessions: [
      {
        id: 'session-1',
