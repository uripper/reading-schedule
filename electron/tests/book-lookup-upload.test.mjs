...
const DATA_URL_PNG_1X1 = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO2+3wAAAABJRU5ErkJggg==';
...
test('saveUploadedCover writes uploaded image to user data directory', () => {
  ...
  try {
    const result = saveUploadedCover(DATA_URL_PNG_1X1, 'book-test', userDataDirectory);
...
test('saveUploadedCover returns empty string for invalid input', () => {
  ...
  try {
    const result = saveUploadedCover('not-a-data-url', 'book-test', userDataDirectory);
    assert.equal(result, '');
  } finally {
    fs.rmSync(userDataDirectory, { recursive: true, force: true });
  }
